...
public class PathPlannerRed extends PathPlanner {
  ...
  /**
   * Load a mirrored path file as a path group. This will separate the path into multiple paths
   * based on the waypoints marked as "stop points"
   *
   * @param name The name of the path group to load
   * @param reversed Should the robot follow this path group reversed
    ...
    for (int i = 0; i < splitWaypoints.size(); i++) {
      ...
      pathGroup.add(
          new PathPlannerTrajectory(
              splitWaypoints.get(i), splitMarkers.get(i), currentConstraints, shouldReverse, true));
   ...
   * Loads a mirrored path
   *
   * @param name The name of the path to be mirrored
