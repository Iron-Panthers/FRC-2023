...
for (const point of (json.waypoints as Waypoint[])) {
  ...
  const newPoint: Waypoint = {
    anchorPoint: { x: FIELD_LENGTH - point.anchorPoint.x, y: point.anchorPoint.y },
    prevControl: point.prevControl ? { x: FIELD_LENGTH - point.prevControl.x, y: point.prevControl.y } : null,
    nextControl: point.nextControl ? { x: FIELD_LENGTH - point.nextControl.x, y: point.nextControl.y } : null,
...
await Deno.writeFile(`${path}.mirror.path`, new TextEncoder().encode(JSON.stringify(newJson, null, 2)));
